...
if audio_file:
    ...
    # Store DCT coefficients in session state to avoid recalculating
    if 'dct_coeffs' not in st.session_state:
        st.session_state.dct_coeffs = dct(signal)
    ...
    ax2.plot(np.abs(st.session_state.dct_coeffs), label="DCT Coefficients", color="red")
    ...
    num_coeffs = st.slider(
        ...
        min_value=1,
        max_value=len(st.session_state.dct_coeffs),
        value=len(st.session_state.dct_coeffs) // 2,
        step=1
    )

    # Reconstruct signal using top N coefficients
    reconstructed_coeffs = np.zeros_like(st.session_state.dct_coeffs)
    reconstructed_coeffs[:num_coeffs] = st.session_state.dct_coeffs[:num_coeffs]
    ...
    fig2, (ax3, ax4) = plt.subplots(2, 1, figsize=(15, 16))
